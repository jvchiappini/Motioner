...
fn render_header(ui: &mut egui::Ui, state: &mut AppState) {
    ui.horizontal(|ui| {
        ui.heading(egui::RichText::new("Project Settings").size(20.0).strong().color(egui::Color32::WHITE));
...
fn render_body(ui: &mut egui::Ui, state: &mut AppState) {
    ...
    egui::Grid::new("settings_grid")
        ...
        .show(ui, |ui| {
            ...
            // Section: Animation
            ui.label(egui::RichText::new("Animation").strong().color(egui::Color32::from_gray(200)));
            ui.end_row();

            ui.label("Frame Rate (FPS)");
            ui.add(egui::DragValue::new(&mut state.fps).clamp_range(1..=240).speed(1));
            ui.end_row();

            ui.label("Duration (seconds)");
            ui.add(egui::DragValue::new(&mut state.duration_secs).clamp_range(0.1..=3600.0).speed(0.1).suffix(" s"));
            ui.end_row();
            ...
            // Section: Output
            ui.label(egui::RichText::new("Output Resolution").strong().color(egui::Color32::from_gray(200)));
            ui.end_row();
...
fn render_footer(ui: &mut egui::Ui, state: &mut AppState) {
    ui.with_layout(egui::Layout::top_down(egui::Align::Center), |ui| {
        ...
        if ui.add_sized(btn_size, egui::Button::new("Close Settings")).clicked() {
            close_settings(state);
        }
    });
}
