...
pub fn show(ui: &mut egui::Ui, state: &mut AppState) {
    ...
    egui::Frame::none()
        ...
        .show(ui, |ui| {
            ...
            let sub_steps = if major_interval >= 60.0 { 6 } else if major_interval >= 1.0 { 10 } else { 5 };
            ...
            for i in (start_step - 1)..=end_step {
                ...
                if t < 0.0 { continue; } // Don't show negative time
                ...
                if x >= rect.left() + gutter_width && x <= rect.right() {
                    // Major tick
                    painter.line_segment(
                        [egui::pos2(x, ruler_rect.top() + 18.0), egui::pos2(x, ruler_rect.bottom())],
                ...
                for j in 1..sub_steps {
                    ...
                    if sub_t < 0.0 { continue; }
                    
                    let sub_x = time_origin_x + sub_t * zoom;
                    if sub_x < rect.left() + gutter_width || sub_x > rect.right() { continue; }
                    
                    let tick_len = if sub_steps > 5 && j == sub_steps / 2 { 8.0 } else { 4.0 };
                    painter.line_segment(
                        [egui::pos2(sub_x, ruler_rect.bottom() - tick_len), egui::pos2(sub_x, ruler_rect.bottom())],
            ...
            for (i, shape) in state.scene.iter().enumerate() {
                ...
                if track_rect.bottom() > tracks_clip_rect.top() && track_rect.top() < tracks_clip_rect.bottom() {
                    ...
                    if bar_rect.width() > 0.0 {
                        ...
                        if is_selected {
                            painter.rect_stroke(bar_rect, 6.0, egui::Stroke::new(1.5, egui::Color32::WHITE));
                        }
                    }
                    
                    // Horizontal separator
                    painter.line_segment(
                        [egui::pos2(rect.left(), track_rect.bottom()), egui::pos2(rect.right(), track_rect.bottom())],
